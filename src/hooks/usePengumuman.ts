 ...
 */
export const usePengumumanDetail = (announcementId: string | undefined) => {
    return useQuery<IApiResponseWithData<IPengumuman>, Error>({
        ...
        queryKey: ["pengumuman", announcementId],
 ...
 */
export const usePengumumanBanner = (announcementId: string | undefined) => {
    return useQuery<Blob, Error>({
        queryKey: ["pengumumanBanner", announcementId],
        queryFn: () => {
            if (!announcementId) {
                return Promise.reject(new Error("Announcement ID is required."));
            }
