      ...
      <div className="w-full bg-white min-h-screen py-4 rounded-sm border-t-2 border-primary-yellow space-y-4">
        ...
        {selectedPeriod && !isError && (
          <StudyResultCardTable
            ...
            data={khsData?.rincianKrs || []}
            ips={khsData?.hasilStudi?.ips}
...
const StudyResultCardTable = ({
  ...
  const totalSKS = data.reduce((sum, item) => sum + (item.kelasKuliah?.mataKuliah?.totalSks || 0), 0);
          ...
          {data.length > 0 ? (
            data.map((item, index) => (
              <tr key={item.id} className="text-center bg-gray-50">
                ...
                <td className="border border-gray-500 px-4 py-2">{item.kelasKuliah?.mataKuliah?.kode}</td>
                <td className="border border-gray-500 px-4 py-2 text-left">{item.kelasKuliah?.mataKuliah?.nama}</td>
                <td className="border border-gray-500 px-4 py-2">{item.kelasKuliah?.mataKuliah?.totalSks}</td>
                <td className="border border-gray-500 px-4 py-2">{item.angkaMutu}</td>
                <td className="border border-gray-500 px-4 py-2">{item.nilaiAkhir}</td>
