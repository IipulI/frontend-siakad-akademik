...
import { getPeriodeAkdemikCoba } from "../../../hooks/academic/usePeriodeAkademikCoba";
...
const emptyRichTextState = convertToRaw(EditorState.createEmpty().getCurrentContent());
...
const AddRps = () => {
  ...
  if (isCourseLoading || isDosenLoading || isCurriculumLoading || isProdiLoading || isPeriodeAkademikLoading) {
    return <LoadingSpinner />;
  }

  if (courseError || dosenError || curriculumError || prodiError || periodeAkademikError) {
  ...
  const handleEditorChange = (name: string, value: RawDraftContentState) => {
    console.log(`Data dari editor '${name}':`, JSON.parse(JSON.stringify(value)));

    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const convertDraftToPlainText = (draftContent: RawDraftContentState): string => {
    try {
      if (!draftContent || !draftContent.blocks || draftContent.blocks.length === 0) {
        return "";
      }
  ...
  const handleSave = async () => {
    if (!formData.siakTahunKurikulumId || !formData.siakPeriodeAkademikId || !formData.siakProgramStudiId || !formData.siakMataKuliahId || !formData.tanggalPenyusun) {
      alert("Harap lengkapi semua field yang wajib diisi (ditandai dengan *)!");
      return;
    }

    const uniqueDosenIds = [...new Set(formData.dosenPenyusun)].filter((id) => id && id.trim() !== "");
  ...
  const selectedTahunKurikulum = curriculumData.find((item) => item.id === formData.siakTahunKurikulumId);
  const selectedPeriodeAkademik = periodeAkademikData.find((item) => item.id === formData.siakPeriodeAkademikId);
  const selectedProgramStudi = prodiData.find((item) => item.id === formData.siakProgramStudiId);
      ...
      <div className="w-full bg-white py-4 rounded-sm border-t-2 border-primary-green px-5 relative">
        <div className="flex items-center justify-end mb-6 mt-4">
          <div className="flex gap-2">
            <button onClick={handleBack} className="bg-primary-yellow text-white px-4 py-2 rounded flex items-center cursor-pointer" disabled={addRpsMutation.isPending}>
              <ArrowLeft className="mr-2" size={16} />
              Kembali ke Daftar
            </button>
            <button onClick={handleSave} className="bg-primary-blueSoft text-white px-4 py-2 rounded flex items-center disabled:opacity-50 cursor-pointer" disabled={addRpsMutation.isPending}>
          ...
          <div className="flex flex-col justify-between bg-[#F5FFF9] p-4 flex-1 md:flex-row md:gap-4">
            ...
            <div className="flex-1">
              <div className="flex justify-between gap-2 md:justify-normal">
                ...
                <div>{selectedPeriodeAkademik?.namaPeriode || "Belum dipilih"}</div>
              </div>
            </div>
            ...
            <div className="flex-1">
              <div className="flex justify-between md:justify-normal">
                ...
                <div>{selectedProgramStudi?.namaProgramStudi || "Belum dipilih"}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="form-group flex flex-col items-start md:items-center gap-4 md:flex-row">
            ...
            <select name="siakMataKuliahId" value={formData.siakMataKuliahId} onChange={handleChange} className="flex-1 p-2 border border-gray-300 rounded w-full" required>
          ...
          <div className="form-group flex flex-col items-start md:items-center md:flex-row gap-4">
            ...
            <input type="date" name="tanggalPenyusun" value={formData.tanggalPenyusun} onChange={handleChange} className="flex-1 p-2 border border-gray-300 rounded w-full" required />
          </div>
            ...
            <div className="space-y-3 w-full">
              {formData.dosenPenyusun.map((dosenId, index) => (
                <div key={index} className="flex items-center gap-2">
                  <select value={dosenId} onChange={(e) => handleChangeDosen(index, e.target.value)} className="flex-1 p-2 border border-gray-300 rounded">
                  ...
                  <button type="button" onClick={() => handleRemoveDosen(index)} className="text-red-600 hover:text-red-800 text-sm p-2">
                    <Trash2 className="w-5 h-5" />
                  </button>
                </div>
              ))}
              <button type="button" onClick={handleAddDosen} className="px-3 py-1 bg-blue-500 text-white text-sm rounded hover:bg-blue-600">
                + Tambah Dosen
              </button>
            </div>
          </div>
          ...
          {[
            ...
            <div key={field.name} className="form-group flex flex-col items-start gap-4 md:flex-row">
              <label className="w-full md:w-48 font-medium pt-2">{field.label}</label>
              <div className="flex-1 w-full">
                <RichTextEditor name={field.name} value={formData[field.name as keyof Omit<RpsFormData, "dokumenRps">]} onChange={handleEditorChange} />
              </div>
            </div>
          ))}
          ...
          <div className="form-group flex flex-col items-start gap-4 mb-4 md:flex-row">
            <label htmlFor="dokumenRps" className="w-full md:w-48 font-medium text-gray-700 pt-2">
              Dokumen RPS
            </label>
